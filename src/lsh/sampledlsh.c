 ...
 */ 
void sampledlsh_get_coitems(ListDB *coitems, HashTable *hash_table)
{
     uint i;
...
{
     HashTable hash_table = l1lsh_create(table_size, tuple_size, listdb->dim, max_value);
     ...
     for (i = 0; i < number_of_tuples; i++){
          ...
          sampledlsh_get_coitems(&coitems, &hash_table);
     }
